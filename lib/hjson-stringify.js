...
module.exports = function($value, $opt) {
  ...
  var needsEscapeML = new RegExp('\'\'\'|^[\\s]+$|[\x00-'+($opt && $opt.multiline === 'std' ? '\x09' : '\x08')+'\x0b\x0c\x0e-\x1f'+commonRange+']', 'g');
  ...
  var indent = '  ';
  // options
  var eol, keepComments, bracesSameLine, multiline, quoteKeys, quoteStrings;
  var token = {
    obj:  [ '{', '}' ],
    arr:  [ '[', ']' ],
    key:  [ '',  '' ],
  ...
  function quote(string, gap, hasComment, isRootObject) {
    ...
    if (quoteStrings || hasComment ||
      ...
      else if (!needsEscapeML.test(string) && !isRootObject && multiline !== 'off') return mlString(string, gap);
  ...
  function str(value, hasComment, noIndent, isRootObject) {
    ...
    var separator = $opt && $opt.separator === true ? ',' : '';
  ...
  function hjsonStringify(value, opt) {
    ...
    if (opt && typeof opt === 'object') {
      ...
      multiline = quoteStrings ? 'off' : (opt.multiline || multiline);
      dsfDef = opt.dsf;
